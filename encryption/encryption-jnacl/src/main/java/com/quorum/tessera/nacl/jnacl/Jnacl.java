...
public class Jnacl implements Encryptor {
	...
	public SharedKey computeSharedKey(final PublicKey publicKey, final PrivateKey privateKey) {
		...
		if (getSharedKey != null) {
			LOGGER.info("get cache shareKeyCache{} and sharedKey {}", getSharedKey, getSharedKey.encodeToBase64());
			return getSharedKey;
		}
	...
	public byte[] seal(final byte[] message, final Nonce nonce, final PublicKey publicKey,
		...
		LOGGER.debug("Sealing message using nonce {}, public key {} and private key {}", nonce, publicKey, privateKey);
		...
		try {
			...
			LOGGER.debug("Created sealed payload using nonce {}, public key {} and private key {}", nonce, publicKey,
					privateKey);
	...
	@Override
	public byte[] open(final byte[] cipherText, final Nonce nonce, final PublicKey publicKey,
			final PrivateKey privateKey) {
		LOGGER.debug("Opening message using nonce {}, public key {} and private key {}", nonce, publicKey, privateKey);
		...
		try {
			...
			LOGGER.debug("Opened message using nonce {}, public key {} and private key {}", nonce, publicKey,
					privateKey);
	...
	public byte[] sealAfterPrecomputation(final byte[] message, final Nonce nonce, final SharedKey sharedKey) {
		...
		final int jnaclResult = secretBox.cryptoBoxAfternm(output, paddedMessage, paddedMessage.length,
				nonce.getNonceBytes(), sharedKey.getKeyBytes());
	...
	public byte[] openAfterPrecomputation(final byte[] cipherText, final Nonce nonce, final SharedKey sharedKey) {
		...
		final int jnaclResult = secretBox.cryptoBoxOpenAfternm(paddedOutput, paddedInput, paddedInput.length,
				nonce.getNonceBytes(), sharedKey.getKeyBytes());
